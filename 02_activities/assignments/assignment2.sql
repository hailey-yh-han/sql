...
SELECT
    COALESCE(product_name, ' ') || ', ' || COALESCE(product_size, ' ') || ', ' || COALESCE(product_qty_type, 'unit') AS formatted_product_list
FROM product;
...
then write another query that uses this one as a subquery (or temp table) and filters the results to only the customer’s most recent visit. */
WITH
    sorted_visits AS (
        SELECT
            customer_id,
...
SELECT product_name, TRIM(
        SUBSTR(
            product_name, INSTR(product_name, '-') + 1
        )
    ) AS description
FROM product
WHERE
...
WITH
    TotalSalesPerDate AS (
        SELECT market_date, SUM(
...
WITH
    VendorCustomerCount AS (
        SELECT v.vendor_name, p.product_name, p.product_id, COUNT(c.customer_id) AS customer_count
        FROM
            vendor v
